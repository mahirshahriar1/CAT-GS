...
from utils.utils import setup_seed, weight_init, unimodal_outputs, evaluate, calculate_accuracy, update_model_with_OGM_GE, modulate_gradients_based_on_scores_from_two_modalities, modulate_gradients_catgs, log_fusion_conflict_joint
...
def train(model, device, train_loader, criterion, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        if config.dataset == 'AVMNIST':
            ...
            # Gray images (modal1) need a channel dimension: [B, 28, 28] -> [B, 1, 28, 28]
            audio = audio.float()
        ...
        # Joint-Train (modulation='Normal'): log fusion-gradient conflict metric per batch.
        # Must happen before backward() because it uses autograd.grad on unimodal losses.
        if config.modulation == 'Normal':
            log_fusion_conflict_joint(epoch=epoch, batch_idx=batch_idx, model=model, out_a=out_a, out_v=out_v, labels=labels, tag='joint')
        ...
        if 'OGM' in config.modulation:
            ...
            elif config.modulation == 'CAT-GS':
                # CAT-GS: calibrated, adaptive, thresholded gating + fusion surgery
                modulate_gradients_catgs(teacher_audio_outputs, teacher_video_outputs, out_a, out_v, labels, model, epoch)
            else:
...
def main():
    ...
    if config.dataset == 'CREMAD':
        ...
        train_dataset = CGMNISTDataset(args=config, mode='train')
        test_dataset = CGMNISTDataset(args=config, mode='test')
    elif config.dataset == 'AVE':
        train_dataset = AVEDataset(args=config, mode='train')
        test_dataset = AVEDataset(args=config, mode='test')
    else:
    ...
    # train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=4, pin_memory=False)
    test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False)
    # test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False, num_workers=4, pin_memory=False)

    # # Use generator for reproducible shuffling across different GPUs
    # g = torch.Generator()
    ...
    start_epoch = 0
    checkpoint_timestamp = current_time  # Use same timestamp for all checkpoints in this run
    
    # Resume from checkpoint if specified
    if config.resume_training and config.resume_checkpoint_path is not None:
        if os.path.exists(config.resume_checkpoint_path):
            print(f"\n=== Resuming training from checkpoint: {config.resume_checkpoint_path} ===")
            checkpoint = torch.load(config.resume_checkpoint_path)
            
            # Load model, optimizer and scheduler states for exact continuity
            ...
            best_path_with_accuracy = checkpoint.get('best_model_path', None)
            # Preserve the original timestamp to keep checkpoint naming consistent
            checkpoint_timestamp = checkpoint.get('checkpoint_timestamp', current_time)
            
            print(f"✓ Loaded checkpoint model from epoch {checkpoint['epoch'] + 1}")
            print(f"Resuming from epoch {start_epoch}")
            print(f"Best accuracy so far: {best_accuracy:.2f}% at epoch {best_epoch + 1}")
            print("=" * 60 + "\n")
        else:
            print(f"\nWarning: Checkpoint path {config.resume_checkpoint_path} does not exist. Starting from scratch.\n")
    ...
    for epoch in tqdm(range(start_epoch, config.epochs)):
            ...
            if epoch == 0:
                if config.modulation == 'G2D':
                    ...
                    with open(os.path.join(config.scores_path, config.filename), 'w') as f:
                        f.write('epoch, coeff_a, coeff_v, score_teacher_a, score_teacher_v, score_a, score_v, weakest_modality, phase\n')
                elif config.modulation == 'OPTION_1' or config.modulation == 'OPTION_2':
                    print(f"Modulation type: {config.modulation}")
                    with open(os.path.join(config.scores_path, config.filename), 'w') as f:
        ...
        if test_accuracy >= best_accuracy:
            ...
            if best_path_with_accuracy is not None and os.path.exists(best_path_with_accuracy):
            ...
            # Save best model with optimizer and scheduler states for proper resumption
            best_checkpoint = {
                'epoch': epoch,
                ...
                'best_model_path': best_path_with_accuracy,
                'train_accuracy': train_accuracy,
                'test_accuracy': test_accuracy,
                'test_accuracy_a': test_accuracy_a,
                'test_accuracy_v': test_accuracy_v,
                'checkpoint_timestamp': checkpoint_timestamp,  # Save timestamp for resume continuity
                'config': {
            ...
            torch.save(best_checkpoint, best_path_with_accuracy)
        ...
        save_checkpoint_every = 20  # Save checkpoint every 50 epochs
        if (epoch + 1) % save_checkpoint_every == 0 or (epoch + 1) == config.epochs:
            checkpoint_path = os.path.join(config.ckpt_path, f'checkpoint_epoch_{epoch+1}_{checkpoint_timestamp}.pth')
            checkpoint = {
                'epoch': epoch,
    ...
    best_checkpoint = torch.load(best_path_with_accuracy)
