...
from utils.utils import setup_seed, weight_init, unimodal_outputs, evaluate, calculate_accuracy, update_model_with_OGM_GE, log_fusion_conflict_joint
...
def train(model, device, train_loader, criterion, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(train_loader_tqdm):
        ...
        current_avg_loss = total_loss / (train_loader_tqdm.n + 1)
...
def main():
    ...
    for epoch in tqdm(range(start_epoch, config.epochs)):                
        ...
        if test_accuracy >= best_accuracy:
            ...
            # Save complete checkpoint (can be used for resuming)
            torch.save({
                'epoch': epoch,
            ...
            }, best_path_with_accuracy)
            print(f"Best model found at epoch {best_epoch+1}, saved to {best_path_with_accuracy} with accuracy {best_accuracy:.2f}%")
        
        print(f"Best Accuracy: {best_accuracy:.2f}%")
        
        # Save checkpoint every 50 epochs for resuming training
        if (epoch + 1) % 50 == 0:
            checkpoint_dir = os.path.join(config.ckpt_path, 'checkpoints')
            os.makedirs(checkpoint_dir, exist_ok=True)
            checkpoint_path = os.path.join(
                checkpoint_dir, 
                f'checkpoint_{config.ckpt_string}_{checkpoint_timestamp}_epoch_{epoch+1}.pth'
            )
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
    ...
    # Load the best model (now it's a full checkpoint)
