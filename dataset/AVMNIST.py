...
class AVMNIST(Dataset):
    def __init__(self,mode = 'train') -> None:
        ...
        print(f"[AVMNIST] Initializing in {mode} mode...")

        image_data_path = os.path.join(config.data_root,'image')
        audio_data_path = os.path.join(config.data_root,'audio')
        
        if mode == 'train':
            print(f"[INFO] Loading image data from {os.path.join(image_data_path, 'train_data.npy')} (mmap_mode='r')")
            self.train_image = np.load(os.path.join(image_data_path, 'train_data.npy'))
            print(f"[INFO] Loading audio data from {os.path.join(audio_data_path, 'train_data.npy')} (mmap_mode='r')")
            self.train_audio = np.load(os.path.join(audio_data_path, 'train_data.npy'))
            print("[INFO] Audio data loaded successfully.")
