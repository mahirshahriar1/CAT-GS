...
class CGMNISTDataset(Dataset):
    ...
    def __init__(self, args, mode='train'):
        self.args = args
        self.mode = mode
        data_root = getattr(args, 'data_root', 'data/Mnist')
        ...
        if self.mode == 'train':
            ...
            self.gray_dataset = datasets.MNIST(root=os.path.join(data_root, 'mnist'), train=True, download=True,
                                               transform=transform)
            ...
            self.gray_dataset = datasets.MNIST(root=os.path.join(data_root, 'mnist'), train=False, download=True,
                                               transform=transform)

        # colored MNIST
        data_dic = np.load(os.path.join(data_root, 'colored_mnist', 'mnist_10color_jitter_var_%.03f.npy' % 0.030),
    ...
    def __getitem__(self, idx):
        ...
        # Return in order: modal1 (gray, like audio), modal2 (colored, like visual), label
